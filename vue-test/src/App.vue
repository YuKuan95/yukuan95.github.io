...
async function getFonts(): Promise<void> {
  const font = new FontFace("TAHOMA", "url(/TAHOMA.ttf)")
...
async function getData() {
  ...
  if (state.resPriceLog?.shortPrice && state.resPriceLog?.nowTime) {
    state.priceArray.push({ time: state.resPriceLog?.nowTime, price: state.resPriceLog?.shortPrice })
  }
